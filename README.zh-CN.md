...
> 本项目 fork 自 [kenanlabs/nav](https://github.com/kenanlabs/nav)；自 v0.8.0 起，内置更新检查基于本仓库自身的 Release/Tag。
...
- 📢 **通知公告** - 后台发布的维护通知/活动预告，前台以可关闭弹窗依次弹出（新公告在前，同一条每位访客仅弹一次）
- 💬 **名言跑马灯** - 顶栏下方吸顶一行跑马灯逐句滚动展示语录（悬停可暂停）
...
- 📢 **通知公告** - 按工作区发布维护通知/活动预告，支持详情链接与生效时间窗，前台以弹窗形式展示
- 📜 **操作审计日志** - 记录登录与站点/分类/工作区/域名/插件/公告/系统设置/管理员变更，保留 90 天，仅超管可见（支持搜索与多维筛选）
...
工作区（Workspace）是独立的内容空间：每个工作区拥有自己的分类、网址、通知公告和展示配置（标题、描述、Logo、Favicon），并可绑定一个或多个域名。访问不同子域名时自动展示对应工作区的内容，未匹配域名时回退到默认工作区。
