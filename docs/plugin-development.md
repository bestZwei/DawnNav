...
- Add the model to `prisma/schema.prisma` (the single SQLite schema)
- Run `npm install` (postinstall regenerates the Prisma client via `scripts/generate-prisma.mjs`)
- Run `npm run db:push` to sync the schema (production syncs automatically at startup via `entrypoint.sh`)
